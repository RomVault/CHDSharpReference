...
namespace CHDReaderTest
{
    internal static class CHDV5Readers
    {
        ...
        internal static chd_error flacSimple(Stream file, int compsize, int hunksize, byte[] cache, out int srcPos)
        {
            byte[] buff = new byte[compsize];
            file.Read(buff, 0, compsize);
        ...
        internal static chd_error cdflac(Stream file, int complen, int destlen, byte[] dest)
        {
            ...
            long filePos = file.Position;
            chd_error err = flacSimple(file, complen, frames * CD_MAX_SECTOR_DATA, bSector, out int pos);
            if (err != chd_error.CHDERR_NONE)
                return err;

            file.Seek(filePos + pos, SeekOrigin.Begin);
            err = zlib(file, 0, frames * CD_MAX_SUBCODE_DATA, bSubcode);
            if (err != chd_error.CHDERR_NONE)
                return err;
